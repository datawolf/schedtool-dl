...
int parse_flags(unsigned int *f, char *arg);
...
int main(int ac, char **dc)
{
        ...
        if(mode_set(mode, MODE_EXEC) && ! (mode_set(mode, MODE_SETPOLICY)
            || mode_set(mode, MODE_AFFINITY) || mode_set(mode, MODE_NICE))) 
        {
...
int set_process(pid_t pid, int policy, struct sched_attr *p, unsigned int flags)
{
        ...
        if((ret = sched_setattr(pid, p, flags))) {
                ...
                if (CHECK_RANGE_POLICY(policy))
                        decode_error(msg1, TAB[policy]);
                else
                        decode_error(msg2, policy);
...
/*
 ...
 than the maximum CPU number that can be stored in cpu_set_t.
 */
static char * cpuset_to_str(cpu_set_t *mask, char *str)
{
        int base;
            ...
            || ((nice = getpriority(PRIO_PROCESS, pid)) && errno)
                ...
                if(! CHECK_RANGE_POLICY(policy)) {
                        ...
                        if (policy == SCHED_DEADLINE) {
                                printf(", RUNTIME %Ldus DEADLINE %Ldus FLAGS 0x%04llx",
                                       p.sched_runtime,
                                       p.sched_deadline,
                                       p.sched_flags);
                        }
